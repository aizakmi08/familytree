...
export default function EditPersonModal({ isOpen, onClose, personId }) {
  ...
  useEffect(() => {
    if (person) {
      setFormData({
        name: person.name || '',
        photo: null,
        photoPreview: person.photo || null,
  ...
  const handleDelete = () => {
    if (window.confirm(`Are you sure you want to delete ${person?.name}? This will also remove all their relationships.`)) {
      const { deletePerson } = useTreeStore.getState();
